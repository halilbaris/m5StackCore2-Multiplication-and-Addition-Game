...
if(fase==0)
 ...
 if (M5.BtnA.wasPressed() || M5.BtnB.wasPressed() || M5.BtnC.wasPressed()) {
    ...
    M5.Lcd.drawString(String(firstN)+" - "+String(secondN)+"      ",70,80); // Substruction
    ...
    for(int i=0;i<4;i++){
    if(answer[i]==0)
    answer[i]=random(0,99); //random numbers limit for multiplaction answers 
    //answer[i]=random(0,9999); //random numbers limit for multiplaction answers 
    //answer[i]=random(0,200); // random numbers limit for addition answers 
